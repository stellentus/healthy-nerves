...
function [brs] = batcher()
	...
	groupStd = std(values);
	valuesShifted = zeroMeanUnitVar(values, groupMean, groupStd);
	canShifted = zeroMeanUnitVar(canValues, groupMean, groupStd);
	japShifted = zeroMeanUnitVar(japValues, groupMean, groupStd);
	porShifted = zeroMeanUnitVar(porValues, groupMean, groupStd);
	legShifted = zeroMeanUnitVar(legValues, groupMean, groupStd);
	...
	brs = getBatchResults(brs, "Normative data", iters, scurr.Seed, @kmeans, 3, valuesShifted, labels);
	...
	brs = getBatchResults(brs, "No Canada", iters, scurr.Seed, @kmeans, 2, [japShifted; porShifted], [ones(japNum, 1); ones(porNum, 1) * 2]);
	brs = getBatchResults(brs, "No Japan", iters, scurr.Seed, @kmeans, 2, [canShifted; porShifted], [ones(canNum, 1); ones(porNum, 1) * 2]);
	brs = getBatchResults(brs, "No Portugal", iters, scurr.Seed, @kmeans, 2, [canShifted; japShifted], [ones(canNum, 1); ones(japNum, 1) * 2]);

	% Confirm that random and perfectly batched data work as expected
	brs = getBatchResults(brs, "Random labels", iters, scurr.Seed, @kmeans, 3, valuesShifted);
	...
	brs = getBatchResults(brs, "Canadian legs", iters, scurr.Seed, @kmeans, 3, [legShifted; japShifted; porShifted], [ones(legNum, 1); ones(japNum, 1) * 2; repmat(3, porNum, 1)]);
