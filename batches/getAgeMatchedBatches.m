...
function [bas] = getAgeMatchedBatches(iters, sampleFraction, filepath)
	...
	bas = [bas BatchAnalyzer("No CA", 2, [japValues; porValues], [ones(size(japValues, 1), 1); ones(size(porValues, 1), 1) * 2], 'iters', iters, 'sampleFraction', sampleFraction)];
	bas = [bas BatchAnalyzer("No JA", 2, [canValues; porValues], [ones(size(canValues, 1), 1); ones(size(porValues, 1), 1) * 2], 'iters', iters, 'sampleFraction', sampleFraction)];
	bas = [bas BatchAnalyzer("No PO", 2, [canValues; japValues], [ones(size(canValues, 1), 1); ones(size(japValues, 1), 1) * 2], 'iters', iters, 'sampleFraction', sampleFraction)];
end
